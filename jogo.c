...
void inicia (void){
       ...
       for(ui32Loop = 0; ui32Loop < 2000000; ui32Loop++){ //delay de +- 2s
       }

       Nokia5110_Clear();
       Nokia5110_DrawFullImage(jogo_velha);
       for(ui32Loop = 0; ui32Loop < 2050000; ui32Loop++){ //delay de +- 2s
       }
...
void partida (void){
    int ocupado[9]={0,0,0,0,0,0,0,0,0};
    char M[9];
    int j=0;
    ...
    while(1){
        ...
        if((GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4) != GPIO_PIN_4) && variavel == 20){ //SW1 1 APERTADO: SELECIONA X
            Nokia5110_Clear();
            ...
            while (j<5){
                ...
                int v = 0;
                            while(aux){
                                ...
                                if ((GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4) != GPIO_PIN_4) && v == 200){
                                    position++;
                                ...
                                if ((GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0) != GPIO_PIN_0) && v == 200){
                                    int x = (position%9);
        ...
        if((GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0) != GPIO_PIN_0) && variavel == 20){ //SW1 2 APERTADO: SELECIONA O
            Nokia5110_Clear();
            Nokia5110_DrawFullImage(tabuleiro);
            while (j<5){
                            playerMov(ocupado, 'O');
...
void setaPosicao(int *ocupado, int x){
    ...
    if(ocupado[x]==0){
        switch (x){
                                case (0):
                                        Nokia5110_SetCursor(30, 10);break;
                                case (1):
                                        Nokia5110_SetCursor(45, 10);break;
                                case (2):
                                        Nokia5110_SetCursor(65, 10);break;
                                case (3):
                                        Nokia5110_SetCursor(30, 20);break;
                                case (4):
                                        Nokia5110_SetCursor(45, 20);break;
                                case (5):
                                        Nokia5110_SetCursor(65, 20);break;
                                case (6):
                                        Nokia5110_SetCursor(30, 25);break;
                                case (7):
                                        Nokia5110_SetCursor(45, 25);break;
                                case (8):
                                        Nokia5110_SetCursor(65, 25);
                                break;
        ...
        for (j=0; j<3; j++){
            if (i==0){
                ...
                if ((v[i][j] == v[i+1][j])&&(v[i+1][j] == v[i+2][j])){
                    ...
                    Nokia5110_Clear(); Nokia5110_OutString ("Voce perdeu!");
                }
                break;
                }
        }else{
         if (j==0){
            //caso em que a vitória ocorre nas linhas.
                if ((v[i][j] ==v[i][j+1])&&(v[i][j+1] == v[i][j+2])){
                   if (x == v[i][j]){
                       Nokia5110_Clear(); Nokia5110_OutString ("Voce ganhou!");
                   }else{
                       Nokia5110_Clear(); Nokia5110_OutString ("Voce perdeu!");
                   }
                    break;
            ...
            //caso em que a vitória ocorre na diagonal principal.
                if((v[i][j] == v[i+1][j+1])&&(v[i+1][j+1] == v[i+2][j+2])){
                   if (x == v[i][j]){
                       Nokia5110_Clear(); Nokia5110_OutString ("Voce ganhou!");
                       }else{
                           Nokia5110_Clear(); Nokia5110_OutString ("Voce perdeu!");
           ...
           if(i == j && j ==1){
            //caso em que a vitória ocorre na diagonal secundária.
                if ((v[i-1][j+1] == v[i,j])&&(v[i][j] == v[i+1][j-1])){
                    if (x == v[i][j]){
                        Nokia5110_Clear(); Nokia5110_OutString ("Voce ganhou!");
                       }else{
                           Nokia5110_Clear(); Nokia5110_OutString ("Voce perdeu!");
